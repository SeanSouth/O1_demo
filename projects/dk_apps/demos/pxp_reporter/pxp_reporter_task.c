...
#else
		0x05, GAP_DATA_TYPE_UUID16_LIST_INC, 0x03, 0x18, // = 0x1803 (LLS UUID)
		0x02, 0x18, // = 0x1802 (IAS UUID)
#endif
		};
...
typedef enum
{
	ADV_INTERVAL_FAST = 0, ADV_INTERVAL_POWER = 1,
} adv_setting_t;
...
static void do_alert(uint8_t level)
{
	...
	{ .dc_min = 0, .dc_max = 255, .freq_div = 255, .polarity =
...
static void lls_alert_cb(uint16_t conn_idx, const bd_address_t *address,
		uint8_t level)
{
	struct device *dev;
...
static void handle_evt_gap_disconnected(ble_evt_gap_disconnected_t *evt)
{
	 ...
	 */
	ble_gap_get_devices(GAP_DEVICE_FILTER_CONNECTED, NULL, &num_connected,
			NULL);
	if (num_connected == 0)
	{
...
void pxp_reporter_task(void *params)
{
	...
	bas_tim = OS_TIMER_CREATE("bas",
			OS_MS_2_TICKS(PX_REPORTER_BATTERY_CHECK_INTERVAL), true,
