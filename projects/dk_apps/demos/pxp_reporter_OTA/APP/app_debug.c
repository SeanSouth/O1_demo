...
#include "..\LIB\mcif\mcif.h"

#include "..\LIB\easylogger\elog.h"

static char tag[]="app_debug";
...
void debug_task(void *params)
{
	...
	while(1)
	{
        ...
        if (notif & MCIF_NOTIF)
        {
        	...
        	{
        		log_i("recv len:%d",rxmsg->len);
        		log_i("recv str:%s",rxmsg->buffer);

        		sprintf(tmpstr,"receive msg!\r\n");
