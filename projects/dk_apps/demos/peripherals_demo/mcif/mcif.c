...
{
        hw_uart_tx_fifo_flush(MCIF_UART);
...
static void uart_init(void)
{
        ...
        hw_uart_set_soft_fifo(MCIF_UART,buffer,50);
}
...
int _write (int fd, char *ptr, int len)
{
        ...
        hw_uart_send(MCIF_UART, ptr, len, NULL, NULL);
        return len;
}
...
int _read (int fd, char *ptr, int len)
{
         ...
         * return 0 from _read()
         */
        while (!hw_uart_is_data_ready(MCIF_UART)) {
        }

        /* and now read as much as possible */
        while (hw_uart_is_data_ready(MCIF_UART) && ret < len) {
                ptr[ret++] = hw_uart_read(MCIF_UART);
        }
